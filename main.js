...
renderer.outputColorSpace = THREE.SRGBColorSpace; // Fixed: updated from deprecated outputEncoding
renderer.shadowMap.enabled = true;
renderer.shadowMap.type = THREE.PCFSoftShadowMap; // Added: better shadow quality
...
sun.castShadow = true;
sun.shadow.mapSize.width = 2048; // Added: better shadow resolution
...
// Fixed: Set initial car position on ground
car.position.y = duneHeight(0, 0) + 0.5; // Start car on ground surface
scene.add(car);
...
function update(dt) {
    ...
    // Fixed: Update car Y position to follow ground height
    car.position.y = duneHeight(car.position.x, car.position.z) + 0.5;
